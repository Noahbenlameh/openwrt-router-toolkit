...
разделе 5 ниже и в `QUICKSTART.md`.
...
## 2. Как пользоваться (что где на странице)

### 2.1. `Status → Wi-Fi Cache Watchdog`

Сверху вниз:

- **Текстовое пояснение** (на русском) — что вкладка делает в целом:
  слушает подключения/отключения к любой Wi-Fi точке и чистит временное
  сетевое состояние роутера для затронутого клиента.
- **Статус-бейджи + кнопки** — `running`/`stopped`, `autostart: on`/`off`,
  и рядом кнопки **Start / Stop / Enable autostart / Disable autostart**
  (дёргают `/etc/init.d/wifi_cache_watchd`).
- **«Результат последней очистки»** — живой индикатор по последнему
  событию: `✓` (зелёный, все шаги реально выполнились), `⚠` (жёлтый,
  что-то пропущено — например нет `conntrack`/`bridge`, подробности в
  логе ниже), `✗` (красный, не смогли определить IP клиента). Рядом — что
  за событие (connect/disconnect), сколько времени назад; MAC — при
  наведении курсора. Это отчёт скрипта о том, что он попытался сделать, а
  не независимая перепроверка состояния таблиц постфактум (для строгой
  проверки — см. `conntrack -L -s <ip>` / `ip neigh show` / `dhcp.leases`
  вручную через терминал).
- **«Cleanup scope»** — три кнопки-переключателя режима: **Only Wi-Fi**
  (по умолчанию, чистит только того клиента, что вызвал событие),
  **Only LAN** (чистит текущих проводных клиентов, классифицированных
  через `bridge fdb`), **Wi-Fi + LAN** (оба сразу). Активный режим
  подсвечен. Под кнопками — предупреждение, если для выбранного режима не
  хватает `conntrack`/`bridge` (с готовой командой `opkg install`).
- **«Live cache state»** — таблица живых счётчиков: записи conntrack
  (текущее/максимум), ARP/ND-записи, активные DHCP-лизы, настроенный
  размер DNS-кэша dnsmasq, время последнего события. Обновляется раз в
  3 сек.
- **«Подключённые устройства»** — таблица всех известных клиентов
  (Wi-Fi + LAN), обновляется раз в 4 сек. Строка сворачивает MAC/тип/IP/
  hostname/сигнал; **клик по строке разворачивает карточку** с полным
  набором: IP, hostname, тип, интерфейс, SSID, канал/частота, сигнал/шум
  (+SNR), RX/TX PHY (скорость, MCS, ширина канала, SGI, число потоков),
  время простоя, «подключён с» (аптайм), 802.11-возможности устройства
  (HT/VHT/HE/WMM/MFP — best-effort, может показывать «unavailable» в
  зависимости от сборки hostapd), срок DHCP-лизы, число и список активных
  соединений (`conntrack`). Всё — пассивное чтение, без обращения к
  самому устройству (см. раздел 7).
- **«Live event log»** — текстовое поле с хвостом лога демона, обновляется
  раз в 2 сек. То же самое, что `logread -e wifi_cache_watchd`, но в
  браузере.

### 2.2. `System → Терминал`

- **Статус-бейджи + кнопки** — `running`/`stopped`, `autostart: on`/`off`,
  `password: set`/`NOT SET` (если не задан пароль — сервис не запустится,
  тут же подсказка с готовой командой). Кнопки **Start / Stop /
  Enable autostart / Disable autostart** (дёргают `/etc/init.d/webconsole`).
- **«Upload file / archive»** — выбор файла + кнопка **Upload to router**.
  Файл всегда ложится под одним и тем же именем — `/tmp/webconsole_upload`
  (см. «Грабли», п.2, почему не динамическое имя), независимо от
  исходного. После загрузки видно точный путь.
- **«Terminal»** — `<iframe>` с самим `ttyd`. При первом открытии спросит
  логин **отдельным окном браузера** — это пароль из `/etc/webconsole.auth`,
  не LuCI-пароль. Если LuCI открыт по https, а `ttyd` по http — нужно один
  раз открыть `http://<router-ip>:7681/` отдельно и принять сертификат,
  иначе `<iframe>` не загрузится (mixed content).
...
flowchart TB
    ...
    subgraph Router["OpenWrt роутер"]
        ...
        subgraph T1["Вкладка «Wi-Fi Cache Watchdog»"]
            ...
            RPCD1["luci.wifi_cache_watchd\n(rpcd: status/log/action/set_mode/clients)"]
            ...
            CONNDIR["/tmp/.wcw_connected/<mac>\n(timestamp коннекта)"]
            RESULT["/tmp/.wcw_last_result\n(ok/partial/failed)"]
            ...
            IWINFO["ubus iwinfo\n(assoclist/info)"] --> RPCD1
            HGC["ubus hostapd.iface\nget_clients (best-effort)"] --> RPCD1
            CONNDIR --> RPCD1
            CTL["conntrack -L -s ip"] --> RPCD1
        end
...
## 5. Подробная инструкция по установке и обновлению
...
## 6. Грабли — почему сделано именно так
...
## 7. Ключевые решения дизайна (почему так, а не иначе)
...
- **Список подключённых устройств собирает только то, что роутер и так уже
  знает пассивно** (iwinfo/hostapd/conntrack — данные, которые устройство
  само сообщило при подключении, или измерения радиочипа на уже идущем
  трафике) — принципиально никаких дополнительных запросов/проб в сторону
  самого устройства. Осознанно не стали делать определение производителя
  по MAC (OUI) — это потребовало бы либо тащить реальную базу IEEE, либо
  руками перечислять префиксы по памяти, что рискует показать неверного
  производителя с уверенным видом.
